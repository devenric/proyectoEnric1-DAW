...
export default function MonigoteBase({ state, onEyeClick }: MonigoteBaseProps) {
  ...
  const renderCabezaSvg = () => {
    ...
    if (selectedCabeza.isUgc && selectedCabeza.ugcUrl) {
      // UGC Render Layer!
      return (
        <foreignObject x="15" y="-12" width="70" height="60" id="ugc-head">
          <img
            src={selectedCabeza.ugcUrl}
            alt="UGC Head"
            style={{ width: '100%', height: '100%', objectFit: 'contain', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.5))' }}
      ...
      case 'gato':
        return (
          <g id="capa-gato" className="origin-bottom">
            {/* Cute cat hood / companion lying on top */}
  ...
  const renderCuerpoSvg = () => {
    if (!selectedCuerpo) return null;

    if (selectedCuerpo.isUgc && selectedCuerpo.ugcUrl) {
      // UGC Render Layer!
      return (
        <foreignObject x="15" y="44" width="70" height="42" id="ugc-body">
          <img
            src={selectedCuerpo.ugcUrl}
            alt="UGC Body"
            style={{ width: '100%', height: '100%', objectFit: 'contain', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.5))' }}
            referrerPolicy="no-referrer"
          />
    ...
    switch (selectedCuerpo.itemCode) {
      ...
      case 'armadura':
        return (
          <g id="capa-armadura">
            {/* Scotch tape cardboard breastplate */}
            ...
            {/* Scribbled crest on shoulder */}
            <rect x="22" y="49" width="4" height="4" fill="#3B82F6" />
            {/* Rusty bottle cap shield/medal */}
  ...
  // Render SVG auras (placed behind or nested to float around the avatar)
  const renderAuraSvg = () => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center p-4">
      ...
      <div className="w-[280px] h-[280px] bg-[#020202] rounded border-2 border-[#ccff00]/30 flex items-center justify-center relative shadow-[inset_0_0_15px_rgba(0,0,0,0.8)] overflow-hidden">
        {/* Pixel Grid lines from stylesheet */}
        <div className="absolute inset-0 pixel-grid opacity-80 pointer-events-none" />

        {/* Dynamic Canvas Container for Monigote Base & Overlays */}
        <motion.div
        ...
        {/* Dynamic Label for current status with neon details */}
        <div className="absolute bottom-3 left-4 right-4 flex justify-between items-center text-[10px] font-mono text-slate-400 select-none bg-black/90 px-3 py-1.5 rounded border border-[#ccff00]/20">
          <span className="text-slate-300">ANIM: {animationState.toUpperCase()}</span>
